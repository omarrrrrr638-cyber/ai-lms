...
const ChatInterface = () => {
    ...
    try {
        console.log(`Sending message for document: ${documentId}`); // <-- تحقق من الـ ID
        const response = await aiService.chat(documentId, userMessage.content);
        console.log("Full API response:", response); // <-- شاهد الرد الكامل من السيرفر
        
        // تأكد من أن المسار إلى الـ answer صحيح
        const assistantMessage = {
            role: 'assistant',
            content: response.data.answer, // قد تحتاج لتغييرها حسب شكل الرد الفعلي
            timestamp: new Date(),
        };
        ...
        console.error('Error response data:', error?.response?.data); // <-- هذا مهم جداً
    } finally {
        setLoading(false);
    }
};
